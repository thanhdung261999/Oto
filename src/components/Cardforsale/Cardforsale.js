...
const Cardforsale = (props) => {
  ...
  useEffect(() => {
    ...
    if (selectedBrandOption && selectModel && selectModel !== 'Choose car model') {
  ...
  const fetListCarWithBrandModel = async () => {
    ...
    const res = await getAllCarWithBrandModel(selectedBrandOption.value, selectModel);
    if (res && res.status === 200) {
      if (res.data.length > 0) {
        setListCar(res?.data);
        setTotal(res?.data?.length);
  ...
  return (
    <div className={`card-for-sale-container ${isTabletAndMobile ? 'container-mb-tb' : 'container'}`}>
      <div className="content ">
        ...
        <div className={`right-content col-lg-8 col-sm-12 col-md-12 col-12`}>
          <div className="title">Cars are being sold at Bussan</div>
          <div className="group-selected">
            <Select
              className="col-lg-4 col-12 col-sm-5 col-md-5 "
            ...
            <select
              ...
              className="col-lg-3 col-12 col-sm-5 col-md-5 select "
              onChange={(e) => {
            ...
            <select
              placeholder="Choose car model"
              className="col-lg-3 col-12 col-sm-5 col-md-5 select select-arrange "
              onChange={(e) => {
          ...
          <Row className="list-car">
            {listCar &&
              ...
              listCar.map((item, index) => {
                return <ProductCar sm="6" md="4" lg="4" xs="6" data={item} key={`item-car-for-fale-${index}`} />;
              })}
          </Row>
        </div>
      </div>
